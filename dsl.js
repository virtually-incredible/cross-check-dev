...
function collectData({
  today,
  status_sheet = get.sheet('accountable statuses'),
  ...
  const subMap = get.subscriptionMap({
    source_sheet,
    statusesRegex,
    adapters,
    year,
  ...
  const statusMap = get.statusMap(
    sheet,
    x['Data row'],
    a1_to_n(x['Ancor column']) - 1,
    a1_to_n(x['Status column']) - 1,
    statusesRegex
  );
...
function checkConsistency(
  ...
  if (m_.length > 1) {
    if (!suppress_output) {
      ...
      ss.toast(
        'Some inconsistency in company names detected. Please check "consistency" tab'
      );
    }
    return false;
  } else {
